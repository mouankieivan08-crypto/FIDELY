...
// au salon. Volontairement très simple : étoiles, commentaire optionnel, envoi.
export default function Avis() {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600" /></div>;
  }

  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-950 to-gray-900 flex flex-col items-center justify-center p-5">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden">
        <div className="bg-gray-950 px-6 py-8 text-center">
          <h1 className="text-2xl font-bold text-white">{business?.name || "JEANNY EMPIRE BEAUTY"}</h1>
          <p className="text-gray-400 text-sm mt-1">Votre avis compte pour nous</p>
        </div>
          ...
          {sent ? (
            ...
            <>
              ...
              <div className="flex justify-center gap-2 mb-6">
                {[1, 2, 3, 4, 5].map(n => (
                  <button key={n} onClick={() => setRating(n)} onMouseEnter={() => setHoverRating(n)} onMouseLeave={() => setHoverRating(0)}
                    ...
                    <Star className={`h-10 w-10 transition-colors ${n <= (hoverRating || rating) ? "fill-amber-400 text-amber-400" : "text-gray-200"}`} />
                  </button>
                ))}
              </div>
              ...
              <button onClick={handleSubmit} disabled={rating === 0 || sending}
                className="w-full py-3.5 bg-gray-950 text-white rounded-xl font-semibold hover:bg-gray-800 disabled:opacity-40 transition-colors">
      ...
      <p className="mt-6 text-gray-500 text-xs">Propulsé par FIDELY</p>
    </div>
  );
}
